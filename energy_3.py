...
teste = np.array([atratores[1], atratores[0]])  #AQUI TEM QUE MUDAR NO SEGUNDO (1 é tratado, 2 é controle 2 e 0 é controle 1)
dhnet.train(teste)
    ...
    if classificacao[p_n] == 1:
        ...
        if marc1 < 75:
            ax.scatter(pontos[p_n][0], pontos[p_n][1], energy(pca.inverse_transform(pontos[p_n])), s=100,
                   c="yellow", label="Treated")
        else:
            ax.scatter(pontos[p_n][0], pontos[p_n][1], energy(pca.inverse_transform(pontos[p_n])), s=100,
                       c="yellow", label="", marker = '^')
...
ax.legend(by_label.values(), by_label.keys(), prop={'size': 16})
...
for p_n in range(len(dados_localizados)):
    if classificacao[p_n] == 1:
        marc4 += 1
        if marc4 < 75:
            ax2.scatter(pontos[p_n][0], pontos[p_n][1], s=100, c="yellow")
        else:
            ax2.scatter(pontos[p_n][0], pontos[p_n][1], s=100, c="yellow", marker = '^')
    elif classificacao[p_n] == 0:
        marc5 += 1
        if marc5 < 48:
            ax2.scatter(pontos[p_n][0], pontos[p_n][1], s=100, c="green")
        else:
            ax2.scatter(pontos[p_n][0], pontos[p_n][1], s=100, c="green", marker = '^')
    elif classificacao[p_n] == 2:
        marc6 += 1
        if marc6 < 18:
            ax2.scatter(pontos[p_n][0], pontos[p_n][1], s=100, c="blue")
        else:
            ax2.scatter(pontos[p_n][0], pontos[p_n][1], s=100, c="blue", marker = '^')
