genes = [] 
for i in range(410):
  if dados_localizados[74][i] != dados_localizados[122][i]:
    genes.append(i)
...
objeto = []
objetivo = [] 
objeto = np.array(dados_localizados[122])
dist= distance.euclidean(objeto, dados_localizados[74])
objetivo = np.array(dados_localizados[74])
print(dist)

if dist > 0: 
  for x in genes:
    objeto[x] = objetivo[x]
    dist = distance.euclidean(objeto, objetivo)
    #print(dist)
    inter.append(x)
    samples.append(np.array(objeto))
    euc.append(dist)

dist = distance.euclidean(objeto, dados_localizados[74])
dist3 = distance.euclidean(objeto, objetivo)

print(dist, dist3, len(inter), len(euc), euc)

list(samples[1])
samples2 = list(samples)
samples3 = []
samples3.append(list(samples2[1]))
for x in range(len(samples2)):
  if x%30 == 0:
    samples3.append(list(samples2[x]))
...
for p_n in range(len(dados_localizados)):
    ...
    elif classificacao[p_n] == 3:
        marc3 += 1 
        if marc3 < 143:
            ax.scatter(pontos[p_n][0], pontos[p_n][1], energy(pca.inverse_transform(pontos[p_n])), s=100,
                       c="black", label="Trajectory", marker = '<')
            
#for i, txt in enumerate(range(len(dados_localizados))):
...
for p_n in range(len(dados_localizados)):
    if classificacao[p_n] == 1:
        ...
        marc6 += 1
        if marc6 < 173:
            ax2.scatter(pontos[p_n][0], pontos[p_n][1], s=100, c="black", marker = '<')

#for i, txt in enumerate(range(len(dados_localizados))):
 # ax2.annotate(txt, (pontos[i][0], pontos[i][1]))
